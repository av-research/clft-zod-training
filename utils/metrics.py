...
def find_overlap_1(n_classes, output, anno):
    '''
    Calculate IoU, precision, recall for all classes, ignoring class 5 (ignore).
    :param n_classes: Number of classes
    :param output: 'fusion' output batch (B, n_classes, H, W)
    :param anno: annotation batch (B, H, W)
    :return: histogram statistic of overlap, prediction and annotation, union for each class
    '''
    # Get predicted class indices
    _, pred_indices = torch.max(output, dim=1)  # (B, H, W)

    # Valid pixels: where annotation is not ignore (class 5)
    valid = (anno != 5)

    # Initialize tensors
    area_overlap = torch.zeros(n_classes, device=output.device)
    area_pred = torch.zeros(n_classes, device=output.device)
    area_label = torch.zeros(n_classes, device=output.device)

    # Compute per class
    for c in range(n_classes):
        area_overlap[c] = ((pred_indices == c) & (anno == c) & valid).sum().float()
        area_pred[c] = ((pred_indices == c) & valid).sum().float()
        area_label[c] = ((anno == c) & valid).sum().float()
